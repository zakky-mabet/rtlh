...
/**
* RTLH
* Mpopulation_excel Class 
*
* @version 1.0.0
* @author Teitra Mega <office@teitramega.co.id>
*/
class Mpopulation_excel extends Rtlh_model 
{
		...
		foreach($this->db->get('penduduk', $limit, $offset)->result() as $key => $value)
		{
			$date = new DateTime($value->tgl_lahir);
			 ...
			 $worksheet->setCellValue('A'.$row_cell, ++$key)
			 		   ...
			 		   ->setCellValue('M'.$row_cell, $value->name)
	...
	{
		return $this->db->get_where('villages', array('slug' => $this->slug->create_slug($param)))->row('id');
	}
}
